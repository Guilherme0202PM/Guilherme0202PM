<!-- HEADER -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=8000e0&height=120&section=header"/>

<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Orbitron&size=45&duration=3000&pause=1000&color=8000E0&background=00000000&center=true&vCenter=true&width=800&height=70&lines=Hi%2C+I'm+Guilherme;Developer+%7C+Data+%7C+AI" alt="Typing SVG" />

</div>

## Hello, welcome, a little about me:

🎓 I'm a final-year Information Systems student at the Federal University of Uberlândia (UFU), with experience as a Junior Database Administrator (DBA), working with SQL structure management, query optimization, and support for development and testing environments.

💡 I'm passionate about software development, data science, and artificial intelligence. Throughout my academic journey and personal projects, I have built solutions using Java, Python, and C++, applying object-oriented principles, modular design, and clean code practices.

🌐 I've developed full-stack web applications using Spring Boot and Angular, created RESTful APIs, built demand forecasting systems, and implemented neural network models using TensorFlow/Keras.

🚀 My goal is to combine solid programming fundamentals with curiosity to solve real-world problems through technology. I believe in writing code that is clean, purposeful, and easy to understand. I'm constantly learning and evolving to contribute to impactful and challenging projects.

<div align="center">
  <a href="https://github.com/Guilherme0202PM">

  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=Guilherme0202PM&show_icons=true&theme=dracula&include_all_commits=true"/>


  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Guilherme0202PM&layout=compact&langs_count=7&theme=dracula"/>

  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Guilherme0202PM&theme=synthwave" alt="Profile Summary" width="80%" />

</div>

<div align="center">
  
  [![Guilherme github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=Guilherme0202PM&bg_color=0d1117&color=c535d0&line=d1056c&point=d1056c&area=true&area_color=d1056c&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

  <img src="https://streak-stats.demolab.com?user=Guilherme0202PM&locale=pt-br&mode=weekly&theme=omni&hide_border=false&border_radius=5&date_format=M%20j%5B,%20Y%5D" height="156" alt="streak graph"  /> <br/>
  <img src="https://github-profile-trophy.vercel.app?username=Guilherme0202PM&theme=omni&column=2&row=1&margin-w=5&margin-h=1&no-frame=false&no-bg=true" height="150" alt="trophy graph"  />
</div>

 

<!-- FOOTER -->
<div align="center">
  <img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg" alt="snake gif" />
</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=8000e0&height=120&section=footer"/>
